...
def generate_new(boardx):
    ...
    for i in range(4):
        for j in range(4):
            if boardx[i][j] == 0:
                newboard = [row[:] for row in boardx]
                newboard[i][j] = 2
                board_list.append(copy.deepcopy(newboard))
                newboard[i][j] = 4
                board_list.append(copy.deepcopy(newboard))
...
class createboardtree():
    ...
    def addbranches(self,i,move,direction):
        board_next_list = self.board_next(self.currentboard.copy(),move)
        if board_next_list:
            for x in board_next_list:
                newboardtree = createboardtree(x,self.order + direction)
